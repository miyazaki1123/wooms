...
// 最初の高さを記録
var previousHeight = body.offsetHeight;
...
var observer = new MutationObserver(function(mutations) {
  mutations.forEach(function(mutation) {
    ...
    if (currentHeight !== previousHeight) {
      previousHeight = currentHeight; // 変更後の高さを更新
      AnimeReset();
    AnimeSetPos();
    }
  });
...
function AnimeOnScroll() {
  ...
  if (AnimePositions.length && AnimeLoadedFlag > 10 ) {
    AnimePositions.forEach(function (data) {
      ...
      }else if (data.flag == 1 && (AnimeData.replay == true || data.replay != null) && data.target == null && data.loaded == null) {
        if (data.offSet > scrollBottom) {
